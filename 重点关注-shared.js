...
// filterKey: 对应筛选下拉的具体标签类型选项，按二级任务命名空间前缀区分——hw-*(作业表现) / essay-*(作文批改) / ability-*(能力变化)，同名标签在不同二级任务下 filterKey 不同（如 hw-emotion 与 essay-emotion）
// 【2026-09-17】情绪心理信号不再区分"高风险/低风险"展示名，统一 filterKey 为 hw-emotion/essay-emotion，内部仍靠 level(l3/l2) 配色、priority(urgent/high) 决定紧急/高角标与排序
...
const data = [
  {
    name:"若宸", cls:"八上9-1班", phone:"13812342201", tier:"A", level:"l3", filterKey:"essay-emotion", perfLabel:"作文批改·情绪心理",
    ...
    evidence:"本次作文《长大以后》被判定为情绪心理信号（优先级：紧急），文中出现自我否定与绝望感表达，原文是：\"我觉得自己一直在拖累大家，有时候真的很想消失，什么都不用管了。\"",
    status:"pending",
    copy:null,
    points:["建议先私下与孩子沟通，了解具体情况，避免在书面反馈中直接提及具体表达内容","建议优先电话联系家长而非文字沟通，语气尽量委婉","请老师结合平时观察自行判断严重程度，并决定后续处理方式"],
    warn:"该条为紧急情绪心理信号，系统仅给出沟通建议要点，请老师自行判断并处理。已同步教研/心理角色纳入周期性复核队列。",
    timingAdvice:"紧急情绪心理信号建议尽快在24小时内沟通，避免拖延；具体时段建议避开孩子在场场合，选择家长独处、方便接听电话的时间。",
  ...
  },
  {
    ...
    timingAdvice:"与同一份作业下的紧急情绪心理任务合并处理，不单独占用一次沟通时机，避免家长短时间内被多次联系。",
    channelAdvice:"建议在处理完风险沟通后，视情况通过文字消息顺带提及即可，无需单独致电；暂无该生历史沟通记录可参考。",
    scriptAdvice:"可作为风险沟通结束后的补充话题，简单提及本次作文评级与可提升方向，不作为独立话术展开。",
    followUps:[]
  },
  {
    name:"啸铭", cls:"八上8-2班", phone:"13934564407", tier:"B", level:"l2", filterKey:"hw-emotion", perfLabel:"作业表现·情绪心理",
    category:"risk", subtype:"emotion", priority:"high", isNew:false, daysAgo:1,
    assignmentTitle:"《我的语文小结》", sourceType:"阅读宝典",
    reason:"本次作文《我的语文小结》中出现消极自我评价（\"我什么都写不好\"），建议关注",
    evidence:"本次作文《我的语文小结》被判定为情绪心理信号（优先级：高），出现消极自我评价，原文是：\"我怎么都写不好，感觉自己写作文就是不行。\"",
    status:"pending",
    ...
    timingAdvice:"该情绪心理信号建议1-2天内沟通，不必占用当天时间，可结合下次自然沟通节点一并提及。",
  ...
  },
  {
    ...
    status:"pending",
    copy:null,
    points:["建议老师私下与孩子沟通确认创作过程，避免在班级或书面反馈中公开质疑","如需联系家长，建议以「想更了解孩子这次写作的想法」为切入点，而非直接质疑","该判定不生成家长沟通文案，比照情绪心理紧急信号的处理方式，仅供老师内部参考"],
    warn:"疑似AI代写涉及对学生的指控性判断，证据不充分时家长反弹风险较高，因此比照情绪心理紧急信号的处理方式，不生成家长沟通话术。",
    timingAdvice:"证据强度高，建议尽快在校内与孩子当面沟通核实，避免拖延导致情况复杂化。",
    channelAdvice:"建议当面沟通为主，比照情绪心理紧急信号场景处理；暂无该生历史沟通记录可参考。",
